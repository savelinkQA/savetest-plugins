...
class PythonParser(BaseParser):
    ...
    def _extract_from_function(
        ...
        # Читаем savetest_case_* комментарии из исходных строк над функцией/декораторами
        case_meta: Dict[str, str] = {}
        case_custom_fields: List[Dict[str, Any]] = []
        if source_lines:
            # Определяем первую строку (1-indexed): либо первый декоратор, либо сама def
            first_line_1based = (
                func_node.decorator_list[0].lineno
                if func_node.decorator_list
                else func_node.lineno
            )
            case_comment_lines = self._collect_comments_before_line(source_lines, first_line_1based)
    ...
    def _collect_comments_before_line(self, source_lines: List[str], line_1based: int) -> List[str]:
        """
        Собирает строки-комментарии непосредственно перед указанной строкой (1-indexed).
        Останавливается на первой пустой строке или не-комментарийной строке.
        Возвращает список в порядке «сверху вниз».
        """
        result = []
        j = line_1based - 2  # 0-indexed, строка перед target
        while j >= 0:
